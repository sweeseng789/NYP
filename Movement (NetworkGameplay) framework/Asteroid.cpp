...
CAsteroid::CAsteroid(std::string type, float locx_, float locy_, float w)
	: w_(0)
	, angular_velocity(0)
	, velocity_x_(0)
	, velocity_y_(0)
...
void CAsteroid::Update(float timedelta)
{
	HGE* hge = hgeCreate(HGE_VERSION);
...
void CAsteroid::Accelerate(float acceleration, float timedelta)
{
...
void CAsteroid::SetName(const char * text)
{
	mytext_.clear();
	mytext_ = text;
}

hgeRect* CAsteroid::GetBoundingBox()
{
...
bool CAsteroid::Update_Laser(std::vector<Laser*> &laserList)
{
...
bool CAsteroid::Update_Bomb(std::vector<CBomb*> &bombList)
{
	for (std::vector<CBomb*>::iterator it = bombList.begin(); it != bombList.end(); ++it)
	{
		CBomb* laser = static_cast<CBomb*>(*it);
		if (hasCollided(laser))
		{
			laser->bombTime = -1.f;
			return true;
...
bool CAsteroid::Update_Explosion(std::vector<Explode*> &explodeList)
{
	for (std::vector<Explode*>::iterator it = explodeList.begin(); it != explodeList.end(); ++it)
	{
		Explode* laser = static_cast<Explode*>(*it);
		if (hasCollided(laser))
		{
			return true;
			break;
...
bool CAsteroid::hasCollided(Explode* explode)
{
	sprite_->GetBoundingBox(x_, y_, &collidebox);
	return collidebox.Intersect(explode->GetBoundingBox());
}

bool CAsteroid::hasCollided(CBomb* bomb)
{
	sprite_->GetBoundingBox(x_, y_, &collidebox);
	return collidebox.Intersect(bomb->GetBoundingBox());
}