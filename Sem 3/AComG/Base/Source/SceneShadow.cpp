...
void SceneShadow::SetParameters()
{
...
float SceneShadow::calculatingFPS(float dt)
{
	return (float)(1.f / dt);
}
...
void SceneShadow::Init()
{
	...
	meshList[GEO_QUAD] = MeshBuilder::GenerateQuad("quad", Color(0.5, 1, 0.3), 10.f);
...
void SceneShadow::SAUpdate(double dt)
{
	SpriteAnimation *sa = dynamic_cast<SpriteAnimation*>(meshList[GEO_SPRITE_ANIMATION]);
...
void SceneShadow::RenderMeshIn2D(Mesh *mesh, bool enableLight, float sizeX, float sizeY, float x, float y, float z)
{
	Mtx44 ortho;
...
void SceneShadow::RenderParticle(Particle * particle)
{
