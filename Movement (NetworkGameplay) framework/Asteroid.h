...
public:
	unsigned int id;
	CAsteroid(std::string type, float locx_, float locy_, float w);
	~CAsteroid();
	...
	bool hasCollided(Explode* explode);
	bool hasCollided(CBomb* bomb);


	bool Update_Laser(std::vector<Laser*> &laserList);
	bool Update_Bomb(std::vector<CBomb*> &bombList);
	bool Update_Explosion(std::vector<Explode*> &explodeList);
