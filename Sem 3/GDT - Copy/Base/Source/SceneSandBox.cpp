...
//	meshList[GEO_AXES] = MeshBuilder::GenerateAxes("reference");//, 1000, 1000, 1000);
	...
	meshList[GEO_HEROSA] = MeshBuilder::Generate2DSpriteAnimation("Hero Sprite Animation", 6, 6, Color(1, 1, 1), 50.f, 50.f);
	meshList[GEO_HEROSA]->textureID = LoadTGA("Image//MasterCheif.tga");
	SpriteAnimation *sa2 = dynamic_cast<SpriteAnimation*>(meshList[GEO_HEROSA]);
	if (sa2)
	{
		sa2->m_anim = new Animation();
		sa2->m_anim->Set(0, 27, 0, 2.f);
	}
...
void SceneSandBox::Update(double dt)
{
	...
	if (sa2)
	{
		...
		static float heroAnimationSpeed = 0.5f;
		if (theHero->GetAnimationInvert() == false)
		{
			//Movement Animation
			if(movingHeroRight == true)
				sa2->m_anim->Set(3, 11, 0, heroAnimationSpeed);
			else
				sa2->m_anim->Set(0, 0, 0, heroAnimationSpeed);

			//Jumping Animation
			if (theHero->isJumpUpwards() == true && theHero->isFreeFall() == false)
				sa2->m_anim->Set(12, 12, 0, heroAnimationSpeed);
			else if (theHero->isJumpUpwards() == false && theHero->isFreeFall() == true)
				sa2->m_anim->Set(13, 13, 0, heroAnimationSpeed);
		}
		else
		{
			//Moving Animation
			if (movingHeroLeft == true)
				sa2->m_anim->Set(16, 25, 0, heroAnimationSpeed);
			else
				sa2->m_anim->Set(14, 14, 0, heroAnimationSpeed);

			//Jumping Animation
			if (theHero->isJumpUpwards() == true && theHero->isFreeFall() == false)
				sa2->m_anim->Set(26, 26, 0, heroAnimationSpeed);
			else if (theHero->isJumpUpwards() == false && theHero->isFreeFall() == true)
				sa2->m_anim->Set(27, 27, 0, heroAnimationSpeed);
		}
	}
}
...
void SceneSandBox::RenderTileMap()
{
	...
	Render2DMesh(meshList[GEO_HEROSA], false, 1.f, theHero->GetPos_x(), theHero->GetPos_y());
