	...
	if (uMsg == WM_SOCKET)
	{
		...
		else
		{
			...
			case FD_ACCEPT:
			{
				...
				welcome += std::to_string(Accept);
				std::string notice = "New Client have joined with ID: ";
				notice += std::to_string(Accept);

				SocketInfo = GetSocketInformation(Accept);
				SocketInfo->DataBuf.buf = const_cast<char *>(notice.c_str());
				SocketInfo->DataBuf.len = notice.length();

				for (std::vector<SOCKET>::iterator it = socketList.begin(); it != socketList.end(); ++it)
				{
					if (*it != SocketInfo->Socket)
					{
						WSASend(*it, &SocketInfo->DataBuf, 1, &SendBytes, 0, NULL, NULL);
					}
				}

				SocketInfo = GetSocketInformation(Accept);
				SocketInfo->DataBuf.buf = const_cast<char *>(welcome.c_str());
				SocketInfo->DataBuf.len = welcome.length();
			...
			case FD_READ:
				...
				else
				{
					...
					else // No error so update the byte count
					{
						...
						std::string msgRecv = SocketInfo->DataBuf.buf;
						std::string id = "";
						std::string msgToSend = "Client " + std::to_string(SocketInfo->Socket) + " sent:";
						int lastLocation = 0;;

						if (msgRecv[0] == '/')
						{
							for (unsigned a = 1; a < msgRecv.size(); a++)
							{
								if (msgRecv[a] != ' ')
								{
									id += msgRecv[a];
							...
							for (unsigned a = lastLocation + 1; a < msgRecv.size(); a++)
							{
								msgToSend += msgRecv[a];
							}
							...
							for (std::vector<SOCKET>::iterator it = socketList.begin(); it != socketList.end(); ++it)
							{
								if (std::to_string(*it) == id)
								{
									SocketInfo->DataBuf.buf = const_cast<char *>(msgToSend.c_str());
									SocketInfo->DataBuf.len = msgToSend.length();
									
									WSASend(*it, &SocketInfo->DataBuf, 1, &SocketInfo->DataBuf.len, 0, NULL, NULL);
			...
			case FD_WRITE:
				...
				if (SocketInfo->BytesRECV > SocketInfo->BytesSEND)
				{
					...
					for (std::vector<SOCKET>::iterator it = socketList.begin(); it != socketList.end(); ++it)
					{
						if (WSASend(*it, &SocketInfo->DataBuf, 1, &SendBytes, 0, NULL, NULL) == SOCKET_ERROR)
						{
...
void CreateSocketInformation(SOCKET s)
{
	...
	static bool isSocketused = false;
	...
	if (isSocketused == false)
	{
		...
		//WSASend(*it, &SocketInfo->DataBuf, 1, &SendBytes, 0, NULL, NULL) == SOCKET_ERROR)

		std::string msgTobeCopy = "Welcome Client with ID: ";
		msgTobeCopy = msgTobeCopy + std::to_string(s);
		char msg[1024];
		strncpy(msg, msgTobeCopy.c_str(), sizeof(msgTobeCopy));
		msg[sizeof(msgTobeCopy)-1] = 0;

		//WSASend(s, msg, 1, 1024, NULL, NULL);
	}
}
