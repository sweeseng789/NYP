...
void Ship::Update(float dt)
{
...
#ifdef INTERPOLATEMOVEMENT
	//Update server angular Position
	serverPos.w += angular_velocity * dt;
	if (serverPos.w > pi)
	{
		serverPos.w -= pi;
	}
	else if (serverPos.w < 0.f)
	{
		serverPos.w += pi;
	}

	//Update client angular position
	clientPos.w += angular_velocity * dt;
	if(clientPos.w > pi)
	{
		clientPos.w -= pi;
	}
	else if (clientPos.w < 0.f)
	{
		clientPos.w += pi;
	}

	pos.w = ratio_ * serverPos.w + (1 - ratio_) * clientPos.w;
#else
	...
	//Server Position Wrap around
	serverPos += serverVel * dt;
	if(serverPos.x < -spritewidth / 2)
	{
		serverPos.x += screenwidth + screenwidth;
	}
	else if(serverPos.x > screenwidth + spritewidth / 2)
	{
		serverPos.x -= spritewidth + spritewidth;
	}

	if(serverPos.y < -spriteheight / 2)
	{
		serverPos.y += spriteheight + spriteheight;
	}
	else if(serverPos.y > spriteheight + spriteheight / 2)
	{
		serverPos.y -= spriteheight + spriteheight;
	}

	//Client Position Wrap around
	clientPos += velocity * dt;
	if(clientPos.x < -spritewidth / 2)
	{
		clientPos.x += screenwidth + screenwidth;
	}
	else if (clientPos.x > screenwidth + screenwidth / 2)
	{
		clientPos.x -= screenwidth + screenwidth;
	}
	...
	else if (clientPos.y > spriteheight + spriteheight / 2)
	{
		clientPos.y -= spriteheight + spriteheight;
	}
